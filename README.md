...
- Merging [Conventional Commits](https://www.conventionalcommits.org) to `main`
  opens a release PR. That PR regenerates `ci-runner-farm.plg` (version entities
  + the package MD5 it verifies) and updates `CHANGELOG.md`.
...
See [docs/RELEASING.md](docs/RELEASING.md) for the full process, the invariants
`release.yml` enforces, and what to check on a release PR.
...
MD5 (the standard Unraid `<FILE>` pattern). Only the `.plg` is committed — the
package is rebuilt reproducibly at publish time.

The scale-set listener in `daemon/` (Go) ships the same way, as a second release
asset installed to `$PLGDIR/bin/crf-scalesetd`. Keeping the binary out of the
tarball is what lets `src/` stay a text-only image and git stay binary-free.
`build-plg.sh` compiles it, so a build needs a Go toolchain; pass `GO` if plain
`go` is not on PATH (e.g. `GO='mise exec -- go' ./build-plg.sh`).

### Layout

```
ci-runner-farm.plg                 installer (generated by the release PR, committed)
build-plg.sh                       packages src/ + builds daemon/ -> versioned .plg
...
daemon/                            crf-scalesetd: Go scale-set listener (own module)
tests/
...
.github/dependabot.yml             gomod updates for daemon/ (scaleset pinned by hand)
```
